...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    upsampled_2_conv7 = tf.layers.conv2d_transpose(layer7_1_1, num_classes,
        kernel_size=4, strides=2, padding="same", activation=tf.nn.relu,
        kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))
    upsampled_2_conv7_with_skipped_connection = tf.add(upsampled_2_conv7, layer4_1_1)

    upsampled_4_conv7 = tf.layers.conv2d_transpose(upsampled_2_conv7_with_skipped_connection,
        num_classes, kernel_size=4, strides=2, padding="same", activation=tf.nn.relu,
        kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))
    upsampled_4_conv7_with_skipped_connection = tf.add(upsampled_4_conv7, layer3_1_1)

    upsampled_8_conv7 = tf.layers.conv2d_transpose(upsampled_4_conv7_with_skipped_connection,
        num_classes, kernel_size=16, strides=8, padding="same", activation=tf.nn.relu,
        kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))

    return upsampled_8_conv7
...
def run():
    ...
    with tf.Session() as sess:
        ...
        get_batches_fn = helper.gen_batch_function(os.path.join(data_dir + image_data_dir, 'training'), image_shape)
